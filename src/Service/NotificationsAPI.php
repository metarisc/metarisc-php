...
class NotificationsAPI extends MetariscAbstract
{
     ...
     */
    public function notificationMarquerCommeLue(string $notification_id) : void
    {
        $table = [
            'notification_id' => $notification_id,
            ];

        $path = preg_replace_callback('/\{([^}]+)\}/', Utils::urlEditor($table), '/notifications/{notification_id}/marquer_comme_lue');
